...
public class CS401BST<E extends Comparable<E>>
{
    ...
    private int hFunc(TreeNode<E> e)
    {
        if (e == null)
            return -1;
        else
            return 1 + Math.max(hFunc(e.get_left()), hFunc(e.get_right()));
    }
    ...
    public void rec_add(E element) {
        root = rec_add(root, new TreeNode<E>(element, root));
        size++;
    }//recursive add method
  
    @SuppressWarnings({ "unchecked", "rawtypes", "unused" })
	private TreeNode<E> rec_add(TreeNode<E> currentParent, TreeNode<E> newNode) {

    	int comp;
    	comp = ((Comparable)currentParent.element).compareTo(newNode.element);
        if (currentParent == null) {
            return newNode;
        } else if (comp < 0) {
            currentParent.right = rec_add(currentParent.right, newNode);
        } else if (comp > 0) {
            currentParent.left = rec_add(currentParent.left, newNode);
